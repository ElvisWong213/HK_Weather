...
struct ContentView: View {
    ...
    @State var changeToListCityView = false
    ...
    var body: some View {
        VStack{
            ...
            .padding(.bottom, 50)
            NineDayWeatherView(getTodayWeatherData: $getTodayWeatherData, getMaxAndMinTempData: $getMaxAndMinTempData, getNineDaysWeatherData: $getNineDaysWeatherData)
            Spacer()
        }
        .sheet(isPresented: $changeToListCityView) {
            ListCityView(getTodayWeatherData: $getTodayWeatherData, ShowLocation: $ShowLocation)
        }
...
struct NineDayWeatherView: View {
    @Binding var getTodayWeatherData: DataStruct.todayWeatherInfo
    @Binding var getMaxAndMinTempData: [String]
    @Binding var getNineDaysWeatherData: DataStruct.nineDaysWeatherInfo
    let weatherIcon = ["Cloudy", "Cold", "Hot", "Raining", "Sunny intervals", "Sunny", "Thunder", "Windy"]
    let weatherIconNumber = [[60, 61, 76, 77, 83, 84, 85], [92, 93], [90, 91], [53, 54, 62, 63, 64], [51, 52, 76], [50, 70, 71, 72, 73, 74, 75], [65], [80, 81, 82]]
